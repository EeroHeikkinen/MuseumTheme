  ...
  <div class="content">
    ...
    <div class="resultHead">
      ...
      <div class="messages">
        ...
        {/if}
        {if $infoMsg}
        <p class="info">{$infoMsg|translate}
          {if $showExport}
          <a class="save" target="_new" href="{$showExport|escape}">{translate text="export_save"}</a>
          {/if}</p>
        {/if}
      </div>
      {/if}
      ...
      <ul class="recordSet">
        ...
        <li class="result{if ($smarty.foreach.recordLoop.iteration % 2) == 0} alt{/if}">
          ...
          {assign var=img_count value=$summImages|@count}
            <div class="coverDiv">
              <div class="resultNoImage">
                <p>{translate text='No image'}</p>
              </div>
          {if $img_count > 0}
              <div class="resultImage">
                <a href="{$url}/Record/{$resource.id|escape:"url"}"><img id="thumbnail_{$summId|escape:"url"} src="{$summThumb|escape}" class="summcover" alt="{translate text='Cover Image'}"/></a>
              </div>
          {else}
              <div class="resultImage">
                <a href="{$url}/Record/{$resource.id|escape:"url"}"><img src="{$path}/images/NoCover2.gif" width="62" height="62" alt="" /></a>
              </div>
          {/if}
