...
{else}
  <form method="get" action="{$path}/Search/Results" name="searchForm" id="searchForm" class="search">
    <div>
      ...
      <input id="searchForm_input" type="text" name="lookfor" size="22" value="{$lookfor|escape}" class="last{if $autocomplete} autocomplete typeSelector:searchForm_type{/if} clearable mainFocus" placeholder="{translate text="Find"}&hellip;"/>
  {if $prefilterList}
