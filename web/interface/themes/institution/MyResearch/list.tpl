...
<div id="resultList" class="myResearch {if $sidebarOnLeft} push-5 last{/if}">
  ...
  <form method="post" name="bulkActionForm" action="{$url}/Cart/Home">
    ...
    <div class="bulkActionButtons">
      <input type="checkbox" class="selectAllCheckboxes floatleft" name="selectAll" id="addFormCheckboxSelectAllBottom"/>
      <label class="floatleft" for="addFormCheckboxSelectAllBottom">{translate text="select_page"}</label>
      <span class="floatleft">|</span>
      <span class="floatleft"><strong>{translate text="with_selected"}: </strong></span>
      {if $bookBag}
      ...
      <input type="submit" class="mail floatleft button" name="email" value="{translate text='Email'}" title="{translate text='email_selected'}"/>
      {if $listEditAllowed}
      <input id="bottom_delete_list_items_{if $list}{$list->id|escape}{/if}" type="submit" class="delete floatleft button" name="delete" value="{translate text='Delete'}" title="{translate text='delete_selected'}"/>
      {/if}
      {if is_array($exportOptions) && count($exportOptions) > 0}
      <input type="submit" class="export floatleft button" name="export" value="{translate text='Export'}" title="{translate text='export_selected'}"/>
      {/if}
