...
<div class="span-18{if $sidebarOnLeft} push-5 last{/if}">
  <div class="toolbar">
    <ul>
      {* TODO: citations <li><a href="{$url}/PCI/Cite?id={$id|escape:"url"}" class="citeRecord PCIRecord cite" id="citeRecord{$id|escape}" title="{translate text="Cite this"}">{translate text="Cite this"}</a></li> *}
      <li><a href="{$url}/PCI/SMS?id={$id|escape:"url"}" class="smsRecord smsPCI sms" id="smsRecord{$id|escape}" title="{translate text="Text this"}">{translate text="Text this"}</a></li>
      <li><a href="{$url}/PCI/Email?id={$id|escape:"url"}" class="mailRecord mailPCI mail" id="mailRecord{$id|escape}" title="{translate text="Email this"}">{translate text="Email this"}</a></li>
      {* TODO: export 
      {if is_array($exportFormats) && count($exportFormats) > 0}
      <li>
        <a href="{$url}/PCI/Export?id={$id|escape:"url"}&amp;style={$exportFormats.0|escape:"url"}" class="export exportMenu">{translate text="Export Record"}</a>
        <ul class="menu offscreen" id="exportMenu">
        {foreach from=$exportFormats item=exportFormat}
          <li><a {if $exportFormat=="RefWorks"}target="{$exportFormat}Main" {/if}href="{$url}/PCI/Export?id={$id|escape:"url"}&amp;style={$exportFormat|escape:"url"}">{translate text="Export to"} {$exportFormat|escape}</a></li>
        {/foreach}
        </ul>
      </li>
      {/if}
      *}
      {* TODO: save
      <li id="saveLink"><a href="{$url}/PCI/Save?id={$id|escape:"url"}" class="saveRecord PCIRecord fav" id="saveRecord{$id|escape}" title="{translate text="Add to favorites"}">{translate text="Add to favorites"}</a></li>
       *}
      {if !empty($addThis)}
  ...
  <div class="record recordId" id="record{$id|escape}">
    ...
    <a href="{$url}/PCI/Search?type=Author&amp;lookfor={$author|escape:"url"}">{$author|escape}</a>{if !$smarty.foreach.loop.last},{/if} 
    {/foreach}
        </td>
      </tr>
      {/if}
    ...
    <a href="{$url}/PCI/Search?type=Author&amp;lookfor={$author|escape:"url"}">{$author|escape}</a>
    {/foreach}
        </td>
      </tr>
      {/if}
    ...
    {foreach from=$record.SubjectTerms item=field name=loop}
      <a href="{$path}/PCI/Search?type=SubjectTerms&amp;lookfor=%22{$field|escape:"url"}%22">{$field|escape}</a><br/>
    {/foreach}
        </td>
      </tr>
      {/if}

      {* TODO: Fix PCI tag support:
      <tr valign="top">
